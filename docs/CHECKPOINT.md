...
- Reproduced and fixed supplied-candidate verification accepting unplanned bytes,
  ...
  agree. `verify --current` now binds the full candidate to regenerated bytes and
  optional owned graph additions, then uses the same coverage requirements as
  application. Both valid candidate forms remain accepted.
- Reproduced and fixed direct-branch edits bypassing structural destination
  checks when their `target` field was omitted. Decoded direct branches and calls
  now require matching, mapped executable target metadata.
- The new regression cases failed in nine places against the prior implementation.
  After the fixes, Debug, Release and LLVM ASan/UBSan each pass **707 checks** in
  all ten CTest programs: foundation 239, cleanup 17, table graph 14, flat mapping
  19, sample pipeline 136, artifacts/process 52, workflow 90, call models 54,
  comparison tree 63 and state expansion 23. Fourteen checks are new; CLI cases
  exercise the public failure status as well as the library result.
- Reinstalled the revised Release build and rebuilt an independent CMake consumer:
